...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   if (!preg_match("/^[a-zA-Z\s]+$/", $name)) {
    ...
    // Process the form if there are no validation errors
    if (!empty($errors)) {
        ...
        $mydb = new myDB();
        $conobj = $mydb->openCon(); // Correct method call
        ...
        if ($result == 1) {
            ...
            $xml_data->addChild('Password', $password); // Avoid saving raw passwords in real applications.
